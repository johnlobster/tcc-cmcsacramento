...
const processContent: (inputData: string) => string = (inputData) => {
  ...
  if (inputData.length === 0) {
    ...
    while (inputDataModified.length > 0) {
      ...
      const match = inputDataModified.match(/<iframe\s+type="text\/html"\s+src="https:\/\/www.youtube.com\/embed\/([^"]+)"\s*>[^<]*<\/iframe>/i)
